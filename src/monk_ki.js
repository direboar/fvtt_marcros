...
(async () => {
  const kiName = "気ポイント";
  const errNoMonkToken = "モンクのトークンを選択してください。";
  ...
  const openHand = !!actor.items.find(
    (o) => o.data.name === "Open Hand Technique"
  )
    ? `<br />In addition, you can impose one of the following: <ul><li>It must succeed on a <b>Dexterity</b> saving throw or be knocked prone.</li><li>It must make a <b>Strength</b> saving throw. If it fails, you can push it up to 15 feet away from you.</li><li>It can’t take reactions until the end of your next turn.</li></ul> Saving throw <b>DC ${
        10 + actor.data.data.abilities.wis.mod
      }</b>`
    : "";
  ...
  const consumeKi = (feature, allowNegative, allowHigher) => {
    ...
    let resourceKey = Object.keys(actor.data.data.resources)
      .filter((k) => actor.data.data.resources[k].label === kiName)
      .shift();
    if (
      resourceKey &&
      (actor.data.data.resources[resourceKey].value >= kiCost || allowNegative)
    ) {
    ...
    actor.items
      .filter(
        (i) =>
          i.data.name === kiName &&
          i.data.hasUses &&
          (i.data.data.uses.value >= kiCost || allowNegative)
      )
      .forEach((i) => {
        hasAvailableResource = true;
  ...
  (async () => {
    ...
    features
      .filter((o) => o.requireLevel <= monkLevels)
      .forEach((o) => {
